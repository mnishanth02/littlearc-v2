...
| `VLT-05` Worker-side file validation | `COMPLETE` | Engineering | Minimized pg-boss dispatch, least-authority authenticated decrypt, bounded JPEG/PNG/static-PDF policy, private staging ClamAV, cleanup-gated terminal state, safe status/logging, disposable Aiven and Railway staging proof pass; HEIC and previews remain fail-closed; see [evidence](./impl-plan/m3-vault-wedge/vlt-05-implementation-evidence.md) |
