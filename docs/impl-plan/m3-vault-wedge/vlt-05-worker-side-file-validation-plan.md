...
- images decode through exact `sharp@0.34.5`, already present in the lockfile;
...
`sharp@0.34.5` is compatible with Node 26 and avoids an unnecessary 0.35
upgrade during this package. The version must still pass the normal
supply-chain review when implementation begins.
...
- `sharp@0.34.5`: already locked transitively; add as an exact direct worker
  dependency only if preview is authorized.
